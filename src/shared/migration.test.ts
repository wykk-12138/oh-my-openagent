...
describe("migrateConfigFile", () => {
  ...
  test("does not write if no migration needed", () => {
   ...
   test("does not auto-migrate legacy model versions in categories", () => {
     ...
     // then: User-selected model is preserved (model-version migration
     // removed in the preference fork to stop overriding user choices)
     expect(needsWrite).toBe(false)
     const categories = rawConfig.categories as Record<string, Record<string, unknown>>
     expect(categories["my-category"].model).toBe("anthropic/claude-opus-4-5")
   })
...
// NOTE: MODEL_VERSION_MAP and migrateModelVersions were removed in the
// preference fork. The plugin no longer rewrites user-configured model
// strings (e.g. openai/gpt-5.3-codex -> openai/gpt-5.4). Tests exercising
// that behavior have been deleted. The sidecar machinery is still
// exercised below via the legacy `_migrations` config path.

describe("migrateConfigFile _migrations tracking", () => {
  test("does nothing when config carries no legacy _migrations field", () => {
    // given: Fresh config with a user-specified model and no migration history
    ...
    // then: User's model choice is preserved and no migration state is written
  ...
  test("preserves user model even when legacy _migrations field is present", () => {
    // given: Config with user-chosen model AND a legacy _migrations array
    // (the preference fork no longer re-applies model-version migrations, so
    // whatever value the user wrote wins).
    const tmpDir = fs.mkdtempSync("/tmp/migration-test-")
    const configPath = `${tmpDir}/oh-my-opencode.json`
    ...
    // then: Model stays exactly as the user configured it
    const hephaestus = (rawConfig.agents as Record<string, Record<string, unknown>>).hephaestus
    expect(hephaestus.model).toBe("openai/gpt-5.3-codex")

    // cleanup
    fs.rmSync(tmpDir, { recursive: true })
  })

  test("strips legacy in-config _migrations field and mirrors it into the sidecar (#3263)", () => {
    // given: Config written by an older plugin version that embedded
    // _migrations inside the config body.
    const tmpDir = fs.mkdtempSync("/tmp/migration-test-")
    const configPath = `${tmpDir}/oh-my-opencode.json`
    const rawConfig: Record<string, unknown> = {
      agents: {
    ...
    // then: The legacy field is stripped from config and persisted to the sidecar
    expect(result).toBe(true)
    expect(rawConfig._migrations).toBeUndefined()
    // User's model choice preserved (no active migration rewrites it)
    expect((rawConfig.agents as Record<string, Record<string, unknown>>).hephaestus.model).toBe(
      "openai/gpt-5.3-codex",
    )
...
// The old `migrateModelVersions with applied migrations` describe block was
// removed along with the migrateModelVersions function itself. Its core
// responsibility (not clobbering user-reverted model choices) is now
// naturally satisfied because the plugin never rewrites model strings.
...
describe("migrateConfigFile with migration tracking via sidecar (#3263)", () => {
  ...
  test("no-op when config has no legacy _migrations field", () => {
    // given: fresh config, user selected their own model
    ...
    // then: no sidecar written, no config rewrite, user model preserved
    expect(needsWrite).toBe(false)
    expect(rawConfig._migrations).toBeUndefined()
    expect(
      (rawConfig.agents as Record<string, Record<string, unknown>>).hephaestus.model,
    ).toBe("openai/gpt-5.3-codex")
    expect(fs.existsSync(sidecarPath(testConfigPath))).toBe(false)
  })

  test("does not rewrite user model even when sidecar records an old migration key", () => {
    // The preference fork: sidecar presence no longer drives any active
    // rewrite. User-configured model is sacred.
    const testConfigPath = tempConfigPath("sidecar-inert")
    fs.writeFileSync(
  ...
  test("mirrors legacy in-config _migrations into the sidecar and strips the field", () => {
    // BC path: configs written by older plugin versions still carry
    // `_migrations` inline. On the next startup we copy that history into
    // the sidecar and remove the inline field so tracking lives in exactly
    // one place.
    ...
    expect(needsWrite).toBe(true)
    expect(rawConfig._migrations).toBeUndefined()
    // User's codex selection preserved (no active migration to rewrite it)
    expect(
      (rawConfig.agents as Record<string, Record<string, unknown>>).hephaestus.model,
    ).toBe("openai/gpt-5.3-codex")
  ...
  test("unions sidecar and legacy _migrations entries, deduplicating", () => {
    // A config written by two different plugin versions may have an entry
    // in both `_migrations` and the sidecar. The merged set must be
    // deduplicated before being persisted back to the sidecar.
    ...
    expect(needsWrite).toBe(true)
    expect(rawConfig._migrations).toBeUndefined()
    // User's opus-4-5 value preserved
    expect(
      (rawConfig.agents as Record<string, Record<string, unknown>>).oracle.model,
